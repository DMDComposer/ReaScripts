...
function isInstrumentTrack(track)
    ...
    for fxIdx = 0, reaper.TrackFX_GetCount(track) - 1 do
        ...
        if string.sub(name, 0, 5) == "VSTi:" then return true end
    end
    return false
end
...
function getScore(track, term)
    ...
    if not instrument and INSTRUMENT_TRACKS_ONLY then return 0 end
    ...
    if not match then
        ...
        if instrument then score = score + 0.1 end
...
function main()
    ...
    if #term == 0 or not term then return end
    ...
    if bestTrack then focusTrack(bestTrack, false) end
    reaper.SetExtState("select_track_by_name", "matches", matches or "", false)
    reaper.SetExtState("select_track_by_name", "current", bestTrackIdx or "",
                       false)
end

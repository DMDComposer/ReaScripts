...
function TextBox(char)
    ...
    if textbox_t.active_char > textbox_t.text:len() then
        textbox_t.active_char = textbox_t.text:len()
    end
end
...
function Run()
    ...
    gfx.rect(obj_offs, obj_offs, obj_mainW - obj_offs * 2,
             gui_fontsize + obj_offs / 2, 1)
    ...
    if char > 0 and textbox_t.text and char ~= 1919379572 and char ~= 1818584692 then
...
function Lokasenna_WindowAtCenter(w, h)
    ...
    local __, __, screen_w, screen_h = reaper.my_getViewport(l, t, r, b, l, t,
                                                             r, b, 1)
